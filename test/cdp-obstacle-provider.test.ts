...
test("every hazard script parses and carries the contract's roles and labels", () => {
  ...
  const externalBlocking = buildDisruptionScript(
    { hazardType: "blocking_modal", targetRole: "primary-action", durationMs: 5_000, intensity: 1 },
    "blocking-external-1",
    { externalSite: true },
  );
  assert.match(externalBlocking, /const externalSite = true/);
  assert.match(externalBlocking, /data-arena-recovery/);
  assert.match(externalBlocking, /close\.textContent = "Close"/);
});
