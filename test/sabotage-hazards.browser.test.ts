...
browserTest("the runner reports decoy clicks and blocked actions against real hazards", async () => {
  ...
  try {
    ...
    // The runner moves a visible cursor onto the target before clicking, so the
    // evidence carries that position alongside what was clicked.
    const { cursor, ...decoyEvidence } = reports[1].evidence ?? {};
    assert.deepEqual(decoyEvidence, {
      target: { role: ROLE, text: "Continue", decoy: true },
      navigated: false,
    });
    assert.equal(cursor?.action, "click");
    assert.ok(
      cursor && cursor.x > 0 && cursor.x <= cursor.viewportWidth && cursor.y > 0 && cursor.y <= cursor.viewportHeight,
      "the cursor lands inside the viewport",
    );
