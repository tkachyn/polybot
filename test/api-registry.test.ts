...
test("a fight created to start now starts at once with its market open", async () => {
  const { factory, prepareCalls } = createFactory();
  const registry = new RaceRegistry(factory);
  const snapshot = await registry.create(raceInput("race-n"), 1_000);
  const race = registry.get("race-n");
  assert.equal(snapshot.race.status, "running");
  assert.equal(race.engine.race.startedAt, 1_000);
  assert.equal(race.market.status, "open");
  assert.equal(prepareCalls.length, 4);
  assert.equal(registry.scheduledStart("race-n"), null);
});
