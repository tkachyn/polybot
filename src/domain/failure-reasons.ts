...
type FailureRule = { pattern: RegExp; cause: string };
...
const FAILURE_RULES: readonly FailureRule[] = [
  ...
  { pattern: /while recovering/i, cause: "stopped while a sabotage still blocked it" },
  { pattern: /\b429\b|rate[ -]?limit|too many requests/i, cause: "stopped after its model provider rate-limited it" },
  { pattern: /budget\b.*\b(?:exhausted|exceeded|spent)\b/i, cause: "stopped when the fight's model budget ran out" },
  {
  ...
  { pattern: /\bmodel\b|openrouter|anthropic|api key/i, cause: "stopped after its model provider failed" },
  { pattern: /exceeded \d+ actions|step budget/i, cause: "stopped after using its whole step budget" },
  {
  ...
  { pattern: /steel|browser session|no (?:active|prepared) session/i, cause: "stopped when its browser session was lost" },
  {
  ...
  { pattern: /runner exited/i, cause: "stopped when its agent quit early" },
  {
